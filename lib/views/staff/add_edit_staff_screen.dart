...
class _AddEditStaffScreenState extends State<AddEditStaffScreen>
 ...
 'receptionist', 'librarian', 'transport'
  ...
  final ok = await context.read<StaffProvider>().addStaff(staff);
  setState(() => _saving = false);
  if (mounted) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(ok ? (widget.isEdit ? 'Staff updated!' : 'Staff added successfully!') : 'Failed to add staff'),
  ...
  onTap: () async {
    final picked = await showDatePicker(context: context,
      initialDate: DateTime(1990), firstDate: DateTime(1950), lastDate: DateTime.now());
  ...
  validator: (v) => (v == null || v.isEmpty) ? 'Date of Birth required' : null,
      ...
      const SizedBox(height: 14),
      _field(_phone, 'Phone Number *', Icons.phone, required: true, type: TextInputType.phone),
      const SizedBox(height: 14),
      _field(_email, 'Email Address *', Icons.email, required: true, type: TextInputType.emailAddress),
      const SizedBox(height: 14),
  ...
  decoration: const InputDecoration(labelText: 'Joining Date *', prefixIcon: Icon(Icons.date_range)),
  onTap: () async {
    final picked = await showDatePicker(context: context,
      initialDate: DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2030));
    if (picked != null) setState(() =>
      _joiningDate.text = '${picked.day.toString().padLeft(2,'0')}/${picked.month.toString().padLeft(2,'0')}/${picked.year}');
  },
  validator: (v) => (v == null || v.isEmpty) ? 'Joining Date required' : null,
      ...
      DropdownButtonFormField<String>(
        value: _department,
        decoration: const InputDecoration(labelText: 'Department *', prefixIcon: Icon(Icons.category)),
      ...
      DropdownButtonFormField<String>(
        value: _role,
        decoration: const InputDecoration(labelText: 'System Role *', prefixIcon: Icon(Icons.security)),
  ...
  Widget _documentsTab() => SingleChildScrollView(
    ...
    child: Column(children: [
      ...
      const Text('Upload required documents for verification', style: TextStyle(color: Colors.grey, fontSize: 13)),
      const SizedBox(height: 20),
